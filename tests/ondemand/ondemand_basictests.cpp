...
namespace dom_api_tests {
  ...
  bool iterate_array() {
    ...
    SUBTEST("ondemand::array", test_ondemand_doc(json, [&](auto doc_result) {
      ...
      int i=0;
      for (SIMDJSON_UNUSED auto value : array) { int64_t actual; value.get(actual); ASSERT_EQUAL(actual, expected_value[i]); i++; }
      ASSERT_EQUAL(i*sizeof(uint64_t), sizeof(expected_value));
      return true;
    }));
    ...
    SUBTEST("ondemand::document", test_ondemand_doc(json, [&](auto doc_result) {
      ...
      int i=0;
      for (SIMDJSON_UNUSED auto value : doc) { int64_t actual; value.get(actual); ASSERT_EQUAL(actual, expected_value[i]); i++; }
      ASSERT_EQUAL(i*sizeof(uint64_t), sizeof(expected_value));
      return true;
    }));
    SUBTEST("simdjson_result<ondemand::document>", test_ondemand_doc(json, [&](auto doc_result) {
      int i=0;
      for (SIMDJSON_UNUSED auto value : doc_result) { int64_t actual; value.get(actual); ASSERT_EQUAL(actual, expected_value[i]); i++; }
      ASSERT_EQUAL(i*sizeof(uint64_t), sizeof(expected_value));
      return true;
    }));
    TEST_SUCCEED();
